...
CREATE TABLE songs (
    ...
    PRIMARY KEY (id),
    FOREIGN KEY (album_id) REFERENCES albums(id) # Mistake Made: Missed (id) on album
);
...
###----- Exercise 2: Select only names of all bands -----###
# Change the name of the column the data returns to Band Name #
SELECT name AS 'Band Name' FROM bands; # Mistake: Typo'd name as names
...
SELECT DISTINCT bands.name AS 'Band Name' FROM bands # Mistake Made: Missing DISTINCT to avoid duplicates
JOIN albums
ON bands.id = albums.band_id; 

###----- Exercise 5: Get all Bands that have No Albums -----###
SELECT DISTINCT bands.name AS 'Band Name'  # A little tough 
FROM bands
LEFT JOIN albums
ON bands.id = albums.band_id
...
###----- Exercise 7: Update the Release Year of the Album with no Release Year -----###
SELECT release_year, id FROM albums; # noticed that album id #4 has a NULL release year
UPDATE albums
...
VALUES (19, 'Bye Bye Bye', 2000, 8); 
SELECT * FROM albums;

###----- Exercise 9: Delete the added Band and Album -----###
DELETE FROM albums
ORDER BY id DESC 
LIMIT 1;
...
SELECT bands.name AS 'Band', COUNT(songs.id) AS 'Number of Songs' # Really tough
FROM bands
